...
export function RootLayout() {
  ...
  const isLandingPage = ['/', '/about', '/security', '/how-it-works', '/documentation', '/privacy', '/support'].includes(location.pathname);
    ...
    <div className="flex min-h-screen flex-col bg-background font-sans antialiased text-foreground selection:bg-primary/20">
          ...
          <div className="flex h-16 items-center justify-between">
            ...
            <div className="flex items-center gap-6">
              ...
              <Link to="/" className="flex items-center gap-2 font-semibold tracking-tight text-lg text-foreground hover:opacity-80 transition-opacity">
                ...
                {!isCollapsed && !isMobileMenuOpen ? "CivicFlow." : <span className="hidden sm:inline">CivicFlow.</span>}
              </Link>

              {isLandingPage && (
                <nav className="hidden md:flex items-center gap-6 text-sm font-medium text-muted-foreground ml-4">
                  <NavLink to="/how-it-works" className={({isActive}) => isActive ? "text-primary font-semibold" : "hover:text-foreground transition-colors"}>How it Works</NavLink>
                  <NavLink to="/about" className={({isActive}) => isActive ? "text-primary font-semibold" : "hover:text-foreground transition-colors"}>About</NavLink>
                  <NavLink to="/security" className={({isActive}) => isActive ? "text-primary font-semibold" : "hover:text-foreground transition-colors"}>Security</NavLink>
                </nav>
              )}
            </div>
            ...
            <div className="flex items-center gap-4">
              <NavLink to="/operations" className={({isActive}) => `text-sm font-medium transition-colors hidden sm:block ${isActive ? "text-primary font-bold" : "text-muted-foreground hover:text-foreground"}`}>
                Workspace
              </NavLink>
              <Link to="/signal">
                <Button size="sm" className="shadow-subtle transition-all hover:-translate-y-0.5 hover:shadow-md">Start AI Investigation</Button>
              </Link>
            </div>
          </div>
        </div>
              ...
              <nav className="flex-1 overflow-y-auto py-6 px-4 space-y-1">
                {isLandingPage ? (
                  <>
                    ...
                    <NavLink to="/security" className={({isActive}) => `flex items-center px-4 py-3 rounded-lg text-sm transition-all ${isActive ? "bg-primary/10 text-primary font-bold border-l-4 border-primary" : "text-muted-foreground hover:bg-muted/50 hover:-translate-y-px hover:shadow-sm border-l-4 border-transparent"}`}>Security</NavLink>
                    <NavLink to="/operations" className={({isActive}) => `flex items-center px-4 py-3 rounded-lg text-sm transition-all ${isActive ? "bg-primary/10 text-primary font-bold border-l-4 border-primary" : "text-muted-foreground hover:bg-muted/50 hover:-translate-y-px hover:shadow-sm border-l-4 border-transparent"}`}>Workspace</NavLink>
                  </>
                ) : (
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        <main className="flex-1 overflow-y-auto w-full scroll-smooth bg-background flex flex-col">
          ...
          {/* Footer inside main container for Workspace layout */}
          {!isLandingPage && (
